...
export class RolesComponent implements OnInit, AfterViewInit {
  ...
  async eliminar(rowElement: HTMLElement, dataId: number) {
    ...
    if (resp.isConfirmed) {
      this.roleService.deleteRole(dataId).subscribe();
      const resp2 = await this.respuestaDialog.fire();
      rowElement.remove();
    }
  }

  editar(roleId: number) {
    this.router.navigate(['roles/edit', roleId]);
  }

  onClickSubmit(roleData: any) {
    // Implement the logic to handle form submission
    console.log('Form data:', roleData);
    // Add your logic to handle the form submission, e.g., call a service to save the role
  }
}
