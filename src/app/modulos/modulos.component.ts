...
export class ModulosComponent implements OnInit, AfterViewInit {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.http
          .post<DataTablesResponse>(
            'http://localhost:8080/modulo/datatable',
  ...
  async eliminar(rowElement: HTMLElement, dataId: number) {
    ...
    if (resp.isConfirmed) {
      this.ModuloService.deleteModulo(dataId).subscribe();
      const resp2 = await this.respuestaDialog.fire();
      rowElement.remove();
    }
  }

  editar(moduloId: number) {
    this.router.navigate(['modulos/edit', moduloId]);
  }

  onClickSubmit(moduloData: any) {
    console.log('Form data:', moduloData);
  }
}
